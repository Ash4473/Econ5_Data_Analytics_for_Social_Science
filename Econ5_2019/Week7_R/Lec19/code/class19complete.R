...
for(n in 1:k){
  if(mtcars$mpg[n]<15){
    efficiency[n] <- "very low"
  } else if(mtcars$mpg[n]<20){
    efficiency[n] <- "low"
  } else if(mtcars$mpg[n]<25){
    efficiency[n] <- "medium"
  } else if(mtcars$mpg[n]<30){
    efficiency[n] <- "high"
  } else efficiency[n] <- "very high"
}
table(efficiency)
...
# 2a. for loops
list.1 = 1:10
# Write a for loop which squares each entry in list.1 and prints the output
for(i in 1:10){
  print(list.1[i]^2)
}
...
#     the Fibonnaci sequence, whose nth element is the sum of
#     the two preceding elements, that is:
#     fibs[n] = fibs[n-1] + fibs[n-2], with F_1 = 1, F_2 = 1.
# Display the vector after your loop to check. Should be 1 1 2 3 5 8 etc.
k <- 20 # This is the size of the sequence. Using 'k' allows us to easily change the size.
fibs <- rep(1, k) # We do this to define the first two entries and to preallocate size for speed
for(n in 3:length(fibs)){
