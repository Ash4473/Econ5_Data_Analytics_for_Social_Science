...
# Your output should look the same as the output from running table(region$region).
...
# b)
region <- _ _ _(california_b = region == "California")
...
#     b) Second, we rename the variable 'salary' to 'california'
salary <- _ _ 
  _(salary_thousands = salary_start_median/1000) _
  _(salary_90 = salary_midcareer_90th)
...
# a)
state_schools <- _ _ _(type == "State")

head(state_schools)

# b)
unique_regions <- _ _ _(region)
...
rich.grads <- # YOUR CODE HERE
  
head(rich.grads)

unique_types <- # YOUR CODE HERE
...
# a)
salary_median <- _ _
  _(name, salary_start_median, salary_midcareer_median)

head(salary_median)

# b)
salary_median_thous <- _ _
  _(name = name, salary_start_thous = salary_start_median/1000, 
...
salary <- _ _ _(salary_start_median)

head(salary) # lowest to highest

salary <- _ _ _(_(salary_start_median))
...
df <- _ _ _(region, by = "name")

head(df)

# After that, we will left join 'type':
df <- _ _ _(type, by = "name")

head(df)

# Note that we could have done both joins in one step:
df <- _ _ _(region, by = "name") _ _ (type, by = "name")
...
df_region <- _ _
  _ (region) _ 
  _(mean_start = mean(salary_start_median), 
